...
async function checkAll() {
  ...
  try {
    ...
    const d = await r.json();
    log('Quick lookup', !!d.definition, d.definition ? '[' + d.source + '] ' + d.definition.slice(0, 55) + '...' : d.error);
  } catch (e) { log('Quick lookup', false, e.message); }
  ...
  } catch (e) { log('Cache working', false, e.message); }
  ...
  try {
    const r = await fetch(base + '/bookmark', {
      method: 'POST', headers,
      body: JSON.stringify({ url: 'https://nodejs.org', title: 'Node.js Docs', topic: 'Node.js', notes: 'Health check' })
    });
    const d = await r.json();
    savedId = d.bookmark?.id;
    log('Bookmark save', d.success, d.success ? 'id=' + savedId + ' saved to Supabase' : d.error);
  } catch (e) { log('Bookmark save', false, e.message); }

  // 5. GET ALL BOOKMARKS
  try {
    const r = await fetch(base + '/bookmarks', { headers });
    const d = await r.json();
    log('Get all bookmarks', Array.isArray(d.bookmarks),
      d.bookmarks?.length + ' bookmark(s) | ' + d.stats?.unique_topics + ' topic(s)');
  } catch (e) { log('Get all bookmarks', false, e.message); }
  ...
  } catch (e) { log('Search bookmarks', false, e.message); }
  ...
  if (savedId) {
    ...
    } catch (e) { log('Delete bookmark', false, e.message); }
  }
  ...
  } catch (e) { log('Dashboard page', false, e.message); }
  ...
  log('Extension files', missing.length === 0,
  ...
  log('Extension icons', missingIcons.length === 0,
